<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8" >
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['info:advertising:add']"
        >新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['info:advertising:edit']"
        >修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['info:advertising:delete']"
        >删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-switch
          v-hasPermi="['info:advertising:switch']"
          @change="handleSwitchChange"
          v-model="advertisingSwitch"
          active-text="广告开关">
        </el-switch>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="是否轮播" align="center" prop="isRotation" :formatter="rotationFormat"  width="80"/>
      <el-table-column label="是否正常" align="center" prop="status" :formatter="statusFormat"  width="80"/>
      <el-table-column label="广告链接" align="center" prop="advertUrl" width="180" show-overflow-tooltip />
      <el-table-column label="点击量" align="center" prop="hits" width="70"/>
      <el-table-column label="排序" align="center" prop="sort" width="70"/>
      <el-table-column label="开始日期" align="center" prop="startTime" width="90">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.startTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
        <el-table-column label="结束日期" align="center" prop="endTime" width="90">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" @close="handleClose" width="600" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="PC封面" >
              <el-tooltip content="图片大小(730*320)" effect="dark" offset="60" placement="top-start">
                <el-upload
                  class="image-upload"
                  :action="upload.url"
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="手机端封面" >
              <el-tooltip content="图片大小(330*220)" effect="dark" offset="60" placement="top-start">
                <el-upload
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="排序号" prop="sort">
              <el-input-number v-model="form.sort" style="width:100%" controls-position="right" :min="1" :precision="0"  label="排序号"></el-input-number>
            </el-form-item>
          </el-col>
        </el-row>
         <el-row>
...
import {allAdvertising, listAdvertising, getAdvertising, addAdvertising, updateAdvertising,delAdvertising} from "@/api/console/info/advertising";
import {updateConfigByKey} from "@/api/console/system/config";
...
export default {
  ...
  created() {
    ...
    this.getConfigKey('adverting_switch').then(response => {
      this.advertisingSwitch = response.msg;
      if( this.advertisingSwitch==='true'){
  ...
  methods: {
    //广告开关修改
    handleSwitchChange(){
      updateConfigByKey({configKey:'adverting_switch',configValue:this.advertisingSwitch}).then(resp=>{
        if(resp.data.code!=200){
          this.$message.error("开关修改失败")
        }
      })
    },
    //上传修改
    ...
    beforePhotoUpload(file){
      ...
      const isJPG = testmsg === 'jpg' || testmsg === 'JPG' ;
    ...
    handleUploadChange(file,fileList){
      if(file.code==200){
        this.$set(this.form,"mobileCover",file.msg);
      }
    },
    //手机端上传成功回调
    handleSuccess(file){
